...
// 貪欲法：スコア最大の列をK列選ぶ
// 引数の cs に結果が入る
void greedy_construction(SCPinstance &pData, SCPsolution &cs)
{
  int maxc;
  ...
  {
    maxc = cs.get_column_maxscore(pData);
...
// 引数の cs に結果が入る
void grasp_construction(SCPinstance &pData, SCPsolution &cs, double alpha)
{
  cs.initialize(pData);
  ...
  for (int k = 1; k <= cs.K; k++)
  {
    c = cs.get_column_grasp(pData, alpha);
                                ...
                                SCPsolution &cs)
{
  ...
  for (int i=1; i <= K; ++i)
  {
    ...
    // 最大スコアの列
    c2 = cs.get_column_maxscore(pData);
                                ...
                                SCPsolution &best_cs)
{
  ...
  {
    // 貪欲法
    greedy_construction(pData, cs);

    // 局所探索
    simple_neighborhood_search(pData, cs);
                               ...
                               SCPsolution &best_cs)
{
  ...
  {
    // 初期解を生成
    grasp_construction(pData, cs, alpha);
...
int main(int argc, char** argv)
{
  ...
  greedy_neighborhood_search(pData, K, niter, CS, Best_CS);
  ...
  grasp_neighborhood_search(pData, K, alpha, niter, CS, Best_graspCS);
